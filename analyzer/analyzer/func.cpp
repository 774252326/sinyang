...
//	dataB.nCycle=p2.noofcycles;
//
//	RefreshDataA(filelist.front(),p3.saplist.front(),dt1,dataB);
//
	...
	)
{
	...
	while(!p3.saplist.empty()){
		...
		if( dataB.ReadTask(p3.saplist.front()) ){
			...
			OneStep(outw,leftp,dt1,dataB);

			x.assign( 1, dataB.additiveVolume );
			y.assign( 1, (dataB.Ar.back()/dataB.Ar0) );

			if(p3.saplist.front().addType==4){
			...
			if(p3.saplist.front().isStepEnd(dataB.Ar.back()/dataB.Ar0)){
	...
	SAPara &p3)
{
	...
	while(!p3.saplist.empty()){
		...
		if( dataB.ReadTask(p3.saplist.front()) ){
			...
			x.assign( 1, dataB.Sml/(dataB.VMSVolume+dataB.additiveVolume) );
