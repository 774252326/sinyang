...
void scalevt(T1 *ov, long n, T2 *nv, T2 k){
	long i;
	for( i=1; i<=n; i++){
		nv[i]=k*ov[i];
	}
...
template <typename T1,typename T2>
void copyvt(T1 *ov, long n, T2 *nv){
	scalevt(ov,n,nv,(T2)1);
}


template <typename T1,typename T2>
void scalemx(T1 **om, long r, long c, T2 **nm, T2 k){
	long i,j;
	for( i=1; i<=r; i++ )
		for( j=1; j<=c; j++ )
			nm[i][j]=k*om[i][j];
...
template <typename T1,typename T2>
void copymx(T1 **om, long r, long c, T2 **nm){
	scalemx(om,r,c,nm,(T2)1);
}
