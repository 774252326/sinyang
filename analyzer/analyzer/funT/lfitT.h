...
template <typename T>
void lfit(T x[], T y[], T sig[], int ndat, T a[], int ia[],
	int ma, T **covar, T *chisq, void (*funcs)(T, T [], long))
	//Given a set of data points x[1..ndat], y[1..ndat] with individual standard deviations
	//sig[1..ndat], use ¦Ö2 minimization to fit for some or all of the coefficients a[1..ma] of
	//a function that depends linearly on a, y =
	//
	//i ai ¡Á afunci(x). The input array ia[1..ma]
	...
	//for a[1..ma], ¦Ö2 = chisq, and the covariance matrix covar[1..ma][1..ma]. (Parameters
	...
	//returns the ma basis functions evaluated at x = x in the array afunc[1..ma].
{
	...
	T ym,wt,sum,sig2i,**beta,*afunc;


	beta=matrix<T>(1,ma,1,1);
	...
	*chisq=0.0;
	for (i=1;i<=ndat;i++) { /*Evaluate ¦Ö2 of the fit.*/
		(*funcs)(x[i],afunc,ma);
