...
//new read data function
void surfaceMesh::loadKWaferLevelContourNodeVector( double **data, long nd, int flag )
{
	...
	//interpolate point value using average 
	for(j=1;j<pointValue.size();j++){
		pointValue[j]/=(double)pcount[j];
	}
...
void surfaceMesh::genBorder(std::vector< std::vector<long> > remainEdge)
{
	std::vector<long> border1;
	std::vector<long> bordere1;
	size_t i;

	while(!remainEdge.empty()){
		border1.clear();
		bordere1.clear();

		border1=remainEdge.front();
		bordere1.push_back(border1.back());
		border1.pop_back();

		remainEdge.erase(remainEdge.begin());


		while( border1.front()!=border1.back() ){
			for(i=0;i<remainEdge.size();i++){
				if(remainEdge[i][0]==border1.back()){
					border1.push_back(remainEdge[i][1]);
					bordere1.push_back(remainEdge[i][2]);
					remainEdge.erase(remainEdge.begin()+i);
					break;
				}
				if(remainEdge[i][1]==border1.back()){
					border1.push_back(remainEdge[i][0]);
					bordere1.push_back(remainEdge[i][2]);
					remainEdge.erase(remainEdge.begin()+i);
					break;
				}
